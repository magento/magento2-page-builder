...
define(['bluefoot/config', 'bluefoot/jquery', 'bluefoot/hook', 'bluefoot/widget/abstract'], function (Config, jQuery, Hook, AbstractField) {
    ...
    InputField.prototype.afterRenderField = function ($hook) {
        this.element.find('input').first().autocomplete({
            source: Config.getPluginConfig('gene_widget_search_app_entity','source_url'),
            appendTo: "#" + this.getId() + "_wrapper",
            minLength: 2,
            select: function(event, ui) {
                jQuery( "#" + this.getId()).val(ui.item.id).data("entity-name", ui.item.value);
            }.bind(this),
    ...
    InputField.prototype.buildHtml = function () {
        ...
        // Get the entity name for the placeholder text
        if( this.entity.data.hasOwnProperty("preview_view") && this.entity.data["preview_view"][this.field.code]["title"] ) {
        ...
        this.element.append(
            ...
            jQuery('<input />').attr({
                name: this.field.code + "_entity_name",
                type: this.getType(),
                id: fakeId
            }).val( placeholder ),
