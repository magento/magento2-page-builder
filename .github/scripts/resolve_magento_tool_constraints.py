...
def npm_install_specs(package_json_path: str) -> tuple[str, str]:
    """
    Read typescript and tslint version constraints from a package.json file and return
    ready-to-use npm install specifiers (e.g. 'typescript@^3.9.7', 'tslint@^5.20.1').
    Falls back to the bare package name when a constraint is not found.
    """
    try:
        data = json.loads(Path(package_json_path).read_text(encoding="utf-8"))
    except Exception as exc:
        print(f"Could not read {package_json_path}: {exc}", file=sys.stderr)
        return "typescript", "tslint"

    deps: dict[str, str] = {
        **data.get("dependencies", {}),
        **data.get("devDependencies", {}),
    }
    ts_c = deps.get("typescript", "")
    tslint_c = deps.get("tslint", "")
    ts_spec = f"typescript@{ts_c}" if ts_c else "typescript"
    tslint_spec = f"tslint@{tslint_c}" if tslint_c else "tslint"
    return ts_spec, tslint_spec
...
def main() -> int:
    ...
    p.add_argument(
        ...
        metavar="PATH",
        help=(
            "Path to the repo's package.json. When provided, resolve npm install specs "
            "for 'typescript' and 'tslint', print '<ts_spec> <tslint_spec>' to stdout, "
            "and exit — PHP resolution is skipped."
        ),
    )
    args = p.parse_args()

    if args.npm_package_json:
        ts_spec, tslint_spec = npm_install_specs(args.npm_package_json)
        print(f"{ts_spec} {tslint_spec}")
        return 0
