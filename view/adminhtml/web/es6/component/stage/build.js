...
export default class Build extends EventEmitter {
     ...
     * @returns {Promise<T>}
     */
    parseAndBuildStage(stageElement) {
     ...
     */
    parseAndBuildElement(element, parent) {
        if (element instanceof HTMLElement &&
            element.getAttribute(Config.getValueAsString('dataRoleAttributeName'))) {
            parent = parent || this.stage;
            let role = element.getAttribute(Config.getValueAsString('dataRoleAttributeName')), data = Build.getElementData(element), children = this.getElementChildren(element);
    ...
    getElementChildren(element) {
        if (element.hasChildNodes()) {
            ...
            _.forEach(element.childNodes, (child) => {
     ...
     * @returns {Promise<T>}
     */
    buildEntity(role, data, parent) {
        return new Promise(function (resolve, reject) {
            createBlock(Config.getContentBlockConfig(role), parent, this.stage, data).then(function (block) {
