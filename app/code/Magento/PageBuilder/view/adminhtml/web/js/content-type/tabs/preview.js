  ...
  function (_previewCollection2) {
    ...
    function Preview(master, config, observableUpdater) {
      ...
      _events.on("tabs:mountAfter", function (args) {
        if (args.contentType.id === _this.master.id && args.expectChildren !== undefined) {
          _this.mountAfterDeferred.resolve(args.expectChildren);
        }
      });

      _events.on("tab-item:mountAfter", function (args) {
        if (_this.element && args.contentType.containerContentType.id === _this.master.id) {
          _this.refreshTabs();
        }
      });
      ...
      _events.on("childContentType:sortUpdate", function (args) {
        if (args.instance.id === _this.master.id) {
    ...
    _proto.addTab = function addTab() {
      ...
      (0, _contentTypeFactory)(_config.getContentTypeConfig("tab-item"), this.master, this.master.stageId).then(function (tab) {
        _events.on("tab-item:mountAfter", function (args) {
          if (args.id === tab.id) {
            _this3.setFocusedTab(_this3.master.children().length - 1);

            _events.off("tab-item:" + tab.id + ":mountAfter");
          }
        }, "tab-item:" + tab.id + ":mountAfter");

        _this3.master.addChild(tab, _this3.master.children().length); // Update the default tab title when adding a new tab


        tab.dataStore.update((0, _translate)("Tab") + " " + (_this3.master.children.indexOf(tab) + 1), "tab_name");
      });
    }
    /**
    ...
    _proto.bindEvents = function bindEvents() {
      ...
      _events.on("tab-item:removeAfter", function (args) {
        if (args.containerContentType.id === _this4.master.id) {
      ...
      _events.on("tab-item:duplicateAfter", function (args) {
        if (_this4.master.id === args.duplicateContentType.containerContentType.id && args.direct) {
      ...
      _events.on("tab-item:mountAfter", function (args) {
        ...
        if (_this4.master.id === args.contentType.containerContentType.id) {
    ...
    _proto.updateTabNamesInDataStore = function updateTabNamesInDataStore() {
      var activeOptions = [];
      this.master.children().forEach(function (tab, index) {
      ...
      this.master.dataStore.update(activeOptions, "_default_active_options");
    }
    /**
