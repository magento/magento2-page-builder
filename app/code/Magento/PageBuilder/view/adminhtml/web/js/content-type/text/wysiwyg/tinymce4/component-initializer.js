...
define(["jquery", "mage/adminhtml/wysiwyg/events"], function (_jquery, _events) {
  ...
  function () {
    ...
    _proto.onFocus = function onFocus() {
      ...
      if ((0, _jquery)(window).width() < this.$element.offset().left + parseInt(this.config.additional.minToolbarWidth, 10)) {
        this.$element.addClass("_right-aligned-toolbar");
      } else {
        this.$element.removeClass("_right-aligned-toolbar");
      }

      _jquery.each(this.config.additional.parentSelectorsToUnderlay, function (i, selector) {
